...
class ProfileManagementRemoteDataSource {
  ...
  Future<void> setUserInApp({
    ...
    try {
      ...
      final body = <String, String>{
        ...
        transactionIdKey: paymentId,
      };

      final response = await http.post(
        Uri.parse(setUserInAppUrl),
        body: body,
        headers: await ApiUtils.getHeaders(),
      );
  ...
  Future<Map<String, dynamic>> getRazorPayDetail() async {
    try {
      final response = await http.post(
        Uri.parse(getRazorpayDetail),
        headers: await ApiUtils.getHeaders(),
      );
      ...
      if (responseJson['status'] == 200) {
        return {
          razorPayApiKey : responseJson[razorPayApiKey],
          razorPaySecretKey : responseJson[razorPaySecretKey],
        };
      } else {
        throw ProfileManagementException(errorMessageCode: 'Error: ${responseJson['status']}');
      }
    } on SocketException catch (_) {
      throw ProfileManagementException(errorMessageCode: errorCodeNoInternet);
    } on ProfileManagementException catch (e) {
